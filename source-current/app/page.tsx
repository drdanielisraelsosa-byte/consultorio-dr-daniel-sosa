...
function ExternalAction({ href, icon, children, tone = "gold", compact = false, sameTab = false }: { href: string; icon: string; children: React.ReactNode; tone?: "gold" | "glass" | "whatsapp"; compact?: boolean; sameTab?: boolean }) {
  return <a className={`external-action ${tone} ${compact ? "compact" : ""}`} href={href} target={sameTab ? "_self" : "_blank"} rel={sameTab ? undefined : "noreferrer"}><Icon name={icon} size={compact ? 17 : 20} /><span>{children}</span>{!compact && <Icon name="arrow-right" size={16} />}</a>;
}
...
export default function Home() {
  ...
  return <main className={`site-shell page-${page}`} onWheel={onWheel} onTouchStart={(event) => { touchStart.current = { x: event.touches[0].clientX, y: event.touches[0].clientY }; }} onTouchEnd={(event) => {
    ...
    <header className="site-header">
      ...
      <div className="header-actions"><ExternalAction href={SITE.links.calendar} icon="calendar" compact sameTab>Agenda</ExternalAction><ExternalAction href={SITE.links.whatsapp} icon="whatsapp" tone="whatsapp" compact>WhatsApp</ExternalAction></div>
    </header>
